import React, { useMemo } from 'react';
import { Grid, Paper, Typography, Box, CircularProgress, Card, CardContent, FormControlLabel, Switch, List, ListItem, ListItemText, ListItemAvatar, Avatar, Chip, Divider, IconButton, InputBase } from '@mui/material';
import { PieChart, Pie, Cell, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, Legend, AreaChart, Area, RadialBarChart, RadialBar } from 'recharts';
import { useData } from '../contexts/DataContext';
import { useColorMode } from '../contexts/ColorModeContext';
import { useTheme, alpha } from '@mui/material/styles';
import { motion } from 'framer-motion';
import { Sector } from 'recharts';
import BugReportIcon from '@mui/icons-material/BugReport';
import VerifiedUserIcon from '@mui/icons-material/VerifiedUser';
import WarningIcon from '@mui/icons-material/Warning';
import AccessTimeIcon from '@mui/icons-material/AccessTime';
import TrendingUpIcon from '@mui/icons-material/TrendingUp';
import SearchIcon from '@mui/icons-material/Search';
import NotificationsNoneIcon from '@mui/icons-material/NotificationsNone';
import SettingsOutlinedIcon from '@mui/icons-material/SettingsOutlined';
import AccountCircleOutlinedIcon from '@mui/icons-material/AccountCircleOutlined';

const COLORS = {
    Critical: '#FF5252', // Neon Red
    High: '#FFAB40',     // Neon Orange
    Medium: '#FFD740',   // Neon Amber
    Low: '#69F0AE',      // Neon Green
    Safe: '#2979FF',     // Electric Blue
    Background: '#050A14'
};

const renderActiveShape = (props: any) => {
    const { cx, cy, innerRadius, outerRadius, startAngle, endAngle, fill, payload, percent } = props;
    return (
        <g>
            <text x={cx} y={cy} dy={-10} textAnchor="middle" fill="#fff" fontWeight="bold" fontSize={14} style={{ filter: 'drop-shadow(0 0 4px rgba(255,255,255,0.5))' }}>
                {payload.name}
            </text>
            <text x={cx} y={cy} dy={15} textAnchor="middle" fill="#94A3B8" fontSize={12}>
                {`${(percent * 100).toFixed(0)}%`}
            </text>
            <Sector
                cx={cx}
                cy={cy}
                innerRadius={innerRadius}
                outerRadius={outerRadius + 8}
                startAngle={startAngle}
                endAngle={endAngle}
                fill={fill}
                style={{ filter: `drop-shadow(0 0 6px ${fill})` }}
            />
            <Sector
                cx={cx}
                cy={cy}
                startAngle={startAngle}
                endAngle={endAngle}
                innerRadius={outerRadius + 10}
                outerRadius={outerRadius + 14}
                fill={fill}
                fillOpacity={0.3}
            />
        </g>
    );
};

const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
        return (
            <Paper
                elevation={0}
                sx={{
                    p: 2,
                    backgroundColor: 'rgba(5, 10, 20, 0.8)',
                    backdropFilter: 'blur(16px)',
                    border: '1px solid rgba(255, 255, 255, 0.1)',
                    borderRadius: 3,
                    boxShadow: '0 8px 32px rgba(0, 0, 0, 0.4)',
                    color: 'text.primary'
                }}
            >
                <Typography variant="subtitle2" sx={{ fontWeight: 800 }}>{label}</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mt: 1 }}>
                    <Box sx={{
                        width: 8,
                        height: 8,
                        borderRadius: '50%',
                        bgcolor: payload[0].fill || payload[0].stroke,
                        boxShadow: `0 0 8px ${payload[0].fill || payload[0].stroke}`
                    }} />
                    <Typography variant="body2" color="text.secondary">
                        {`${payload[0].name}:`}
                    </Typography>
                    <Typography variant="body2" sx={{ fontWeight: 'bold', color: '#fff' }}>
                        {payload[0].value.toLocaleString()}
                    </Typography>
                </Box>
            </Paper>
        );
    }
    return null;
};

export const Dashboard: React.FC = () => {
    const { data, loading } = useData();
    const { mode, toggleColorMode } = useColorMode();
    const theme = useTheme();
    const [activeIndex, setActiveIndex] = React.useState(0);

    const onPieEnter = (_: any, index: number) => {
        setActiveIndex(index);
    };

    const stats = useMemo(() => {
        if (loading) return null;

        const severityCount: Record<string, number> = { Critical: 0, High: 0, Medium: 0, Low: 0 };
        const riskCount: Record<string, number> = {};

        data.forEach(item => {
            if (item.severity in severityCount) severityCount[item.severity]++;
            item.riskFactors?.forEach(risk => riskCount[risk] = (riskCount[risk] || 0) + 1);
        });

        const severityData = Object.entries(severityCount).map(([name, value]) => ({ name, value }));
        const riskData = Object.entries(riskCount)
            .map(([name, value]) => ({ name, value }))
            .sort((a, b) => b.value - a.value)
            .slice(0, 5);

        const trendData = [
            { name: 'Jan', count: Math.floor(data.length * 0.1) },
            { name: 'Feb', count: Math.floor(data.length * 0.15) },
            { name: 'Mar', count: Math.floor(data.length * 0.12) },
            { name: 'Apr', count: Math.floor(data.length * 0.2) },
            { name: 'May', count: Math.floor(data.length * 0.18) },
            { name: 'Jun', count: Math.floor(data.length * 0.25) },
        ];

        // Detailed Metrics
        const total = data.length;
        const critical = severityCount.Critical;
        const high = severityCount.High;
        const fixRate = 84.5;
        const mttr = "4h 23m";
        const healthScore = Math.max(0, Math.min(100, 85));

        const recentActivity = data.slice(0, 5).map((item, i) => ({
            id: i,
            cve: item.cveId,
            pkg: item.packageName,
            severity: item.severity,
            time: `${Math.floor(Math.random() * 23) + 1}h ago`
        }));

        return { severityData, riskData, trendData, total, critical, high, fixRate, mttr, healthScore, recentActivity };
    }, [data, loading]);

    if (loading || !stats) {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh' }}>
                <CircularProgress size={60} thickness={4} />
            </Box>
        );
    }

    const cardVariants = {
        hidden: { opacity: 0, y: 20 },
        visible: { opacity: 1, y: 0, transition: { duration: 0.5 } }
    };

    return (
        <Box sx={{ pb: 4 }}>
            {/* Header Area */}
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', md: 'center' }, mb: 5, gap: 2 }}>
                <Box>
                    <Typography variant="h4" sx={{ fontWeight: 800, mb: 0.5, textShadow: '0 0 20px rgba(41, 121, 255, 0.3)' }}>Security Overview</Typography>
                    <Typography variant="body1" color="text.secondary">Real-time insights and performance metrics.</Typography>
                </Box>

                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    {/* Search Bar */}
                    <Paper
                        elevation={0}
                        sx={{
                            p: '2px 4px',
                            display: 'flex',
                            alignItems: 'center',
                            width: 300,
                            borderRadius: '12px',
                            bgcolor: alpha(theme.palette.background.paper, 0.5),
                            border: `1px solid ${alpha(theme.palette.divider, 0.1)}`
                        }}
                    >
                        <IconButton sx={{ p: '10px' }} aria-label="search">
                            <SearchIcon />
                        </IconButton>
                        <InputBase
                            sx={{ ml: 1, flex: 1 }}
                            placeholder="Search CVEs, Assets..."
                            inputProps={{ 'aria-label': 'search cves' }}
                        />
                    </Paper>

                    <IconButton sx={{ bgcolor: alpha(theme.palette.background.paper, 0.5), borderRadius: 3 }}>
                        <NotificationsNoneIcon />
                    </IconButton>
                    <IconButton sx={{ bgcolor: alpha(theme.palette.background.paper, 0.5), borderRadius: 3 }}>
                        <SettingsOutlinedIcon />
                    </IconButton>
                    <FormControlLabel
                        control={<Switch checked={mode === 'dark'} onChange={toggleColorMode} />}
                        label=""
                        sx={{ m: 0 }}
                    />
                    <Avatar sx={{ bgcolor: theme.palette.primary.main, boxShadow: `0 0 10px ${theme.palette.primary.main}` }}>
                        <AccountCircleOutlinedIcon />
                    </Avatar>
                </Box>
            </Box>

            {/* KPI Cards Row */}
            <Grid container spacing={3} sx={{ mb: 4 }}>
                {[
                    { title: 'Security Score', value: `${stats.healthScore}/100`, sub: 'Excellent', icon: <VerifiedUserIcon />, color: COLORS.Safe },
                    { title: 'Critical Issues', value: stats.critical.toLocaleString(), sub: '-12% vs last week', icon: <WarningIcon />, color: COLORS.Critical },
                    { title: 'Fix Rate', value: `${stats.fixRate}%`, sub: 'Top 10% Industry', icon: <TrendingUpIcon />, color: COLORS.Low },
                    { title: 'MTTR', value: stats.mttr, sub: 'Mean Time', icon: <AccessTimeIcon />, color: '#00E5FF' }
                ].map((item, index) => (
                    <Grid key={index} size={{ xs: 12, sm: 6, md: 3 }} component={motion.div} initial="hidden" animate="visible" variants={cardVariants} custom={index}>
                        <Card
                            elevation={0}
                            sx={{
                                p: 2,
                                height: '100%',
                                position: 'relative',
                                overflow: 'visible',
                                // Note: card styling handled by theme, adding specific overrides for glow
                                '&:hover': {
                                    boxShadow: `0 8px 24px -4px ${alpha(item.color, 0.25)}`,
                                    borderColor: alpha(item.color, 0.3),
                                    transform: 'translateY(-2px)',
                                    transition: 'all 0.3s ease'
                                }
                            }}
                        >
                            <Box sx={{
                                position: 'absolute', top: -10, right: 15,
                                p: 1, borderRadius: '12px',
                                background: `linear-gradient(135deg, ${alpha(item.color, 0.2)} 0%, ${alpha(item.color, 0.1)} 100%)`,
                                border: `1px solid ${alpha(item.color, 0.2)}`,
                                boxShadow: `0 0 15px ${alpha(item.color, 0.2)}`,
                                color: item.color
                            }}>
                                {item.icon}
                            </Box>
                            <CardContent>
                                <Typography variant="subtitle2" color="text.secondary" sx={{ fontWeight: 600 }}>{item.title}</Typography>
                                <Typography variant="h3" sx={{ fontWeight: 800, my: 1, letterSpacing: '-0.03em', textShadow: `0 0 20px ${alpha(item.color, 0.3)}` }}>{item.value}</Typography>
                                <Chip
                                    size="small"
                                    label={item.sub}
                                    sx={{
                                        bgcolor: alpha(item.color, 0.1),
                                        color: item.color,
                                        fontWeight: 700,
                                        borderRadius: '6px',
                                        border: `1px solid ${alpha(item.color, 0.2)}`
                                    }}
                                />
                            </CardContent>
                        </Card>
                    </Grid>
                ))}
            </Grid>

            {/* Main Visuals Grid */}
            <Grid container spacing={4}>
                {/* Left Column: Health & Recent Activity */}
                <Grid size={{ xs: 12, md: 4, lg: 3 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {/* Health Score Gauge */}
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                            <Paper elevation={0} sx={{ p: 3, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                                {/* Decorative background blur */}
                                <Box sx={{ position: 'absolute', top: '-50%', left: '10%', width: '80%', height: '80%', background: `radial-gradient(${alpha(COLORS.Safe, 0.2)} 0%, transparent 70%)`, filter: 'blur(40px)', zIndex: 0 }} />

                                <Typography variant="h6" sx={{ fontWeight: 700, mb: 2, position: 'relative', zIndex: 1 }}>System Health</Typography>
                                <Box sx={{ height: 200, position: 'relative', zIndex: 1 }}>
                                    <ResponsiveContainer width="100%" height="100%">
                                        <RadialBarChart
                                            innerRadius="70%"
                                            outerRadius="100%"
                                            data={[{ name: 'Score', value: stats.healthScore, fill: COLORS.Safe }]}
                                            startAngle={180}
                                            endAngle={0}
                                        >
                                            <RadialBar background={{ fill: alpha(theme.palette.divider, 0.1) }} dataKey="value" cornerRadius={30} style={{ filter: `drop-shadow(0 0 10px ${COLORS.Safe})` }} />
                                        </RadialBarChart>
                                    </ResponsiveContainer>
                                    <Box sx={{ position: 'absolute', top: '60%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
                                        <Typography variant="h3" sx={{ fontWeight: 800, color: COLORS.Safe, textShadow: `0 0 20px ${COLORS.Safe}` }}>{stats.healthScore}</Typography>
                                        <Typography variant="caption" color="text.secondary">/ 100</Typography>
                                    </Box>
                                </Box>
                                <Typography variant="body2" color="text.secondary" sx={{ mt: -4, position: 'relative', zIndex: 1 }}>Your system is verified safe.</Typography>
                            </Paper>
                        </motion.div>

                        {/* Recent Activity Feed */}
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
                            <Paper elevation={0} sx={{ p: 0, overflow: 'hidden' }}>
                                <Box sx={{ p: 3, borderBottom: `1px solid ${alpha(theme.palette.divider, 0.1)}` }}>
                                    <Typography variant="h6" sx={{ fontWeight: 700 }}>Recent Activity</Typography>
                                </Box>
                                <List sx={{ p: 0 }}>
                                    {stats.recentActivity.map((activity, i) => (
                                        <React.Fragment key={i}>
                                            <ListItem alignItems="flex-start" sx={{ px: 3, py: 2.5, '&:hover': { bgcolor: alpha(theme.palette.primary.main, 0.05) }, transition: 'all 0.2s' }}>
                                                <ListItemAvatar>
                                                    <Avatar sx={{
                                                        bgcolor: alpha(COLORS[activity.severity as keyof typeof COLORS], 0.1),
                                                        color: COLORS[activity.severity as keyof typeof COLORS],
                                                        boxShadow: `0 0 10px ${alpha(COLORS[activity.severity as keyof typeof COLORS], 0.3)}`
                                                    }}>
                                                        <BugReportIcon fontSize="small" />
                                                    </Avatar>
                                                </ListItemAvatar>
                                                <ListItemText
                                                    primary={<Typography variant="subtitle2" sx={{ fontWeight: 600 }}>{activity.cve}</Typography>}
                                                    secondary={
                                                        <Box component="span" sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.5 }}>
                                                            <Typography variant="caption" color="text.secondary">{activity.pkg}</Typography>
                                                            <Typography variant="caption" sx={{ fontWeight: 600, color: alpha(theme.palette.text.primary, 0.5) }}>{activity.time}</Typography>
                                                        </Box>
                                                    }
                                                />
                                            </ListItem>
                                            {i < stats.recentActivity.length - 1 && <Divider component="li" variant="inset" sx={{ borderColor: alpha(theme.palette.divider, 0.05) }} />}
                                        </React.Fragment>
                                    ))}
                                </List>
                            </Paper>
                        </motion.div>
                    </Box>
                </Grid>

                {/* Right Column: Charts */}
                <Grid size={{ xs: 12, md: 8, lg: 9 }}>
                    <Grid container spacing={4}>
                        {/* Trend Area Chart (Full Width) */}
                        <Grid size={{ xs: 12 }}>
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
                                <Paper elevation={0} sx={{ p: 4, height: 360 }}>
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                                        <Typography variant="h6" sx={{ fontWeight: 700 }}>Detection Trends</Typography>
                                        <Chip label="Last 6 Months" size="small" sx={{ borderRadius: 1, bgcolor: alpha(theme.palette.primary.main, 0.1), color: theme.palette.primary.main }} />
                                    </Box>
                                    <ResponsiveContainer width="100%" height="85%">
                                        <AreaChart data={stats.trendData}>
                                            <defs>
                                                <linearGradient id="colorCount" x1="0" y1="0" x2="0" y2="1">
                                                    <stop offset="5%" stopColor="#2979FF" stopOpacity={0.4} />
                                                    <stop offset="95%" stopColor="#2979FF" stopOpacity={0} />
                                                </linearGradient>
                                            </defs>
                                            <XAxis dataKey="name" stroke={theme.palette.text.secondary} fontSize={12} tickLine={false} axisLine={false} dy={10} />
                                            <YAxis hide />
                                            <Tooltip content={<CustomTooltip />} />
                                            <Area
                                                type="monotone"
                                                dataKey="count"
                                                stroke="#2979FF"
                                                strokeWidth={3}
                                                fillOpacity={1}
                                                fill="url(#colorCount)"
                                                animationDuration={2000}
                                                style={{ filter: 'drop-shadow(0 0 10px rgba(41, 121, 255, 0.4))' }}
                                            />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </Paper>
                            </motion.div>
                        </Grid>

                        {/* Split: Pie Breakdown & Risk Factors */}
                        <Grid size={{ xs: 12, md: 6 }}>
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.4 }}>
                                <Paper elevation={0} sx={{ p: 4, height: 400 }}>
                                    <Typography variant="h6" sx={{ mb: 4, fontWeight: 700 }}>Severity Distribution</Typography>
                                    <ResponsiveContainer width="100%" height="85%">
                                        <PieChart>
                                            <Pie
                                                {...{ activeIndex } as any}
                                                activeShape={renderActiveShape}
                                                data={stats.severityData}
                                                cx="50%" cy="50%"
                                                innerRadius={80} outerRadius={110}
                                                paddingAngle={4}
                                                dataKey="value"
                                                onMouseEnter={onPieEnter}
                                                cornerRadius={6}
                                            >
                                                {stats.severityData.map((entry: any, index: number) => (
                                                    <Cell
                                                        key={`cell-${index}`}
                                                        fill={COLORS[entry.name as keyof typeof COLORS] || '#888'}
                                                        stroke="none"
                                                        style={{ filter: activeIndex === index ? `drop-shadow(0 0 10px ${COLORS[entry.name as keyof typeof COLORS]})` : 'none' }}
                                                    />
                                                ))}
                                            </Pie>
                                            <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ paddingTop: '20px' }} />
                                        </PieChart>
                                    </ResponsiveContainer>
                                </Paper>
                            </motion.div>
                        </Grid>

                        <Grid size={{ xs: 12, md: 6 }}>
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.5 }}>
                                <Paper elevation={0} sx={{ p: 4, height: 400 }}>
                                    <Typography variant="h6" sx={{ mb: 4, fontWeight: 700 }}>Top Risk Factors</Typography>
                                    <ResponsiveContainer width="100%" height="85%">
                                        <BarChart data={stats.riskData} layout="vertical" margin={{ left: 20 }}>
                                            <defs>
                                                <linearGradient id="barGradient" x1="0" y1="0" x2="1" y2="0">
                                                    <stop offset="0%" stopColor="#00E5FF" stopOpacity={0.4} />
                                                    <stop offset="100%" stopColor="#00E5FF" stopOpacity={1} />
                                                </linearGradient>
                                            </defs>
                                            <XAxis type="number" hide />
                                            <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 11, fill: theme.palette.text.secondary }} axisLine={false} tickLine={false} />
                                            <Tooltip content={<CustomTooltip />} cursor={{ fill: alpha(theme.palette.action.hover, 0.2) }} />
                                            <Bar
                                                dataKey="value"
                                                fill="url(#barGradient)"
                                                radius={[0, 6, 6, 0]}
                                                barSize={20}
                                                animationDuration={1500}
                                                style={{ filter: 'drop-shadow(0 0 6px rgba(0, 229, 255, 0.4))' }}
                                            />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </Paper>
                            </motion.div>
                        </Grid>
                    </Grid>
                </Grid>
            </Grid>
        </Box>
    );
};
